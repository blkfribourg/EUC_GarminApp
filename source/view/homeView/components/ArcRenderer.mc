...
class ArcRenderer extends WatchUi.Drawable {
    ...
    function draw(dc) {
        ...
        switch (mArcType) {
            case :speedArc: {
                if (currentValue != 0.0) {
            if (currentValue >= AppStorage.getSetting("OrangeColoringThreshold") && currentValue < AppStorage.getSetting("RedColoringThreshold")) {
                foregroundColor = mSecondColor;
            } else if (currentValue >= AppStorage.getSetting("RedColoringThreshold")) {
            ...
            case :batteryArc: {
              ...
              var batteryPercentage=eucData.getBatteryPercentage();
                ...
                if(currentValue >= maxValue) {
                    ...
                    var percentage = computedPercentageLoadDrop.toFloat() / maxValue.toFloat();
